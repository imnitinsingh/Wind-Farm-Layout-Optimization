...
if __name__ == "__main__":
    ...
    # turb_coords   =  getTurbLoc(r'C:\Users\Nitin.N.Singh\Desktop\HACK\Optimization\GA\Data\turbine_loc_test2.csv')
    ...
    n_runs = 10000
    rand_sol_fitness = np.zeros((n_runs,1), dtype=np.float32)
    for n_ in tqdm(range(n_runs)):
        r"""
        dummy_solution = np.array([0]*int(50) + [1]*50)
        np.random.shuffle(dummy_solution)
        sample_sol = dummy_solution.reshape(10,10)
            
        #---------fill up the turbine coords---------------------
        turb_coords = np.zeros((50,2),dtype = np.float32)
        k = 0
        # fill in the x and y coords based on the sample sol
        for i in range(10): # loop over row of sample_sol
            for j in range(10): # loop over col of sample_sol  
                if sample_sol[i,j] == 1: # now calc the coords
                    turb_coords[k,0] = (j*2 + 1)*200
                    turb_coords[k,1] = (i*2 + 1)*200
                    k+=1
        #--------------------------------------------------------
        """
        turb_coords   =  getTurbLoc(r'C:\Users\Nitin.N.Singh\Desktop\HACK\Optimization\GA\Data\rand config\50 Turbs\{0}.csv'.format(n_+1))
        # isFeasible = checkConstraints(turb_coords, turb_diam)
        # if isFeasible ==False: break
        AEP = getAEP(turb_rad, turb_coords, power_curve, wind_inst_freq, 
                  n_wind_instances, cos_dir, sin_dir, wind_sped_stacked, C_t) 
        
        rand_sol_fitness[n_,0] = AEP
